...
console.log({ reduce: reduce([4, 1, 3], function(a,b) {return a * b}, 1)})

// Challenge 7
function intersection(arrays) {
    return arrays.reduce((result, current) => {
        return current.filter((value) => result.includes(value))
    })
...
function multiMap(arrVals, arrCallbacks) {
    ...
    for (let i = 0; i < arrVals.length; i++) {
        ...
        for(const callback of arrCallbacks) {
            resultArr.push(callback(arrVals[i]))
        }

        resultMap[arrVals[i]] = resultArr;
    }
...
function majority(array, callback) {
    ...
    const majorityCount = parseInt(array.length / 2) + 1;

    for (const element of array) {
        (callback(element)) ? responseMap.truthy++ : responseMap.falsy++;
        const { truthy } = responseMap;
        if (truthy === majorityCount) {
            response = true;
            break;
        }
    }
...
function prioritize(array, callback) {
    ...
    const filtered = array.filter(element => !result.includes(element))
...
function groupBy(array, callback) {
    ...
    for(const element of array) {
        ...
        (result[key] === undefined) ? result[key] = [element] : result[key].push(element);
    }
...
function goodKeys(obj, callback) {
    ...
    for (const property in obj) {
        const isTrue = callback(obj[property]);
        if (isTrue) {
            responseTrue.push(property);
        }
    }
...
function objFilter(obj, callback) {
    ...
    for (const property in obj) {
        if (callback(parseInt(property)) === obj[property]) {
...
function highestFunc(objOfFuncs, subject) {
    ...
    for(const property in objOfFuncs) {
        const result = objOfFuncs[property](subject);
        if (result > highest) {
            highest = result;
            outputKey = property;
        }
